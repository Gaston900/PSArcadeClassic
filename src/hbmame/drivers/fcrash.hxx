...
GAME( 1993, dinopic2,  dino,     dinopic,   dino,     cps_state, init_dinopic,  ROT0,   "bootleg", "Cadillacs and Dinosaurs (bootleg with PIC16C57, set 2)", MACHINE_NO_SOUND | MACHINE_SUPPORTS_SAVE ) // 930201 ETC
...
void cps_state::captcommb2hb_map(address_map &map) {
...
MACHINE_START_MEMBER(cps_state, captcommb2hb)
{
	u8 *ROM = memregion("audiocpu")->base();
...
void cps_state::captcommb2hb(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 10000000);
	m_maincpu->set_addrmap(AS_PROGRAM, &cps_state::captcommb2hb_map);
	m_maincpu->set_vblank_int("screen", FUNC(cps_state::cps1_interrupt));
	m_maincpu->set_addrmap(m68000_base_device::AS_CPU_SPACE, &cps_state::cpu_space_map);
	...
	MCFG_MACHINE_START_OVERRIDE(cps_state, captcommb2hb)
	MCFG_MACHINE_RESET_OVERRIDE(cps_state, fcrash)
...
GAME( 1991, captcommb2hb,captcomm, captcommb2hb,captcomm, cps_state, init_cps1,     ROT0,   "bootleg", "Captain Commando (bootleg set 2)(bootleg with YM2151 + 2xMSM5205)", MACHINE_SUPPORTS_SAVE )
...
/*********************************************************************************
                      Developed By Remikare [kuroma_mameui]
********************************************************************************************/

 /********************************************
        Added CPS1 bootleg (not working)
**********************************************/
...
GAME( 1991,  captcommb2, captcomm, captcommb2hb,captcomm, cps_state, init_cps1,   ROT0,  "bootleg",  "Captain Commando (bootleg with 2xMSM5205)",  MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE )  // 911014 ETC
...
GAME( 1993, mtwinsb,   mtwins,     mtwinsb,   mtwins,     cps_state, init_mtwinsb,  ROT0,   "David Inc. (bootleg)",             "Twins (Mega Twins bootleg)",  MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE ) // based on World version
...
void cps_state::ffightblb_sound_map(address_map &map) {
	map(0x0000, 0x7fff).rom();
...
GAME( 1990, ffightblb,  ffight,  ffightblb, fcrash,    cps_state,   init_cps1,      ROT0,    "bootleg (Soon Hwa)",  "Final Fight (bootleg with single OKI)", MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE ) // priority glitches
...
GAME( 1992, varthb,     varth,   varthb,    varth,     cps_state,   init_mtwinsb,   ROT270,  "bootleg",  "Varth: Operation Thunderstorm (bootleg, set 1)",  MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE )
...
//	map(0x88000a, 0x88000f).r(FUNC(cps_state::sgyxz_dsw_r));            /* Dip Switches */
	map(0x890000, 0x890001).w(FUNC(cps_state::cps1_soundlatch2_w));
	map(0x900000, 0x92ffff).ram().w(FUNC(cps_state::cps1_gfxram_w)).share("gfxram");
	map(0xf1c004, 0xf1c005).w(FUNC(cps_state::cpsq_coinctrl2_w));     /* Coin control2 (later games) */
...
void cps_state::wofr1bl_layer_w(offs_t offset, u16 data)
{
	switch (offset)
	{
	case 0x00:
...
void cps_state::wofr1bl_layer2_w(u16 data)
{
	m_cps_a_regs[0x06 / 2] = data;
}


void cps_state::wofr1bl(machine_config &config)
{
	sgyxz(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &cps_state::wofr1bl_map);
	EEPROM_93C46_8BIT(config, "eeprom");
	MCFG_MACHINE_START_OVERRIDE(cps_state, wofr1bl)
	MCFG_MACHINE_RESET_REMOVE()
}


void cps_state::wofr1bl_map(address_map &map) {
	map(0x000000, 0x3fffff).rom();
	map(0x800000, 0x800007).portr("IN1");            /* Player input ports */
	map(0x800006, 0x800007).w(FUNC(cps_state::cps1_soundlatch_w));    /* Sound command */
	map(0x800008, 0x800009).w(FUNC(cps_state::wofr1bl_layer2_w));
	map(0x800018, 0x80001f).r(FUNC(cps_state::cps1_dsw_r));            /* System input ports / Dip Switches */
	map(0x800030, 0x800037).w(FUNC(cps_state::cps1_coinctrl_w));
	map(0x800100, 0x80013f).w(FUNC(cps_state::cps1_cps_a_w)).share("cps_a_regs");  /* CPS-A custom */
	map(0x800140, 0x80017f).rw(FUNC(cps_state::cps1_cps_b_r), FUNC(cps_state::cps1_cps_b_w)).share("cps_b_regs");  /* Only writes here at boot */
	map(0x880000, 0x880001).nopw(); // ?
	map(0x900000, 0x92ffff).ram().w(FUNC(cps_state::cps1_gfxram_w)).share("gfxram");
	map(0x980000, 0x98000d).w(FUNC(cps_state::wofr1bl_layer_w));
...
MACHINE_START_MEMBER(cps_state, wofr1bl)
{
	m_layer_enable_reg = 0x26;
	m_layer_mask_reg[0] = 0x28;
	m_layer_mask_reg[1] = 0x2a;
...
void cps_state::init_wofr1bl()
{
	m_bootleg_sprite_ram = std::make_unique<u16[]>(0x2000);
	m_maincpu->space(AS_PROGRAM).install_ram(0x990000, 0x993fff, m_bootleg_sprite_ram.get());
//	m_maincpu->space(AS_PROGRAM).install_write_handler(0x990000, 0x990001, write16smo_delegate(*this, FUNC(cps_state::wofr1bl_spr_base_w)));
	init_cps1();
}
...
GAME( 1992, wofr1bl,    wof,     wofabl,    wofabl,    cps_state, init_cps1,   ROT0,    "bootleg",  "Warriors of Fate (bootleg)", MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE )   // 921002 ETC


u16 cps_state::slampic2_cps_a_r(offs_t offset)
{
...
void cps_state::slampic_layer2_w(offs_t offset, u16 data, u16 mem_mask)
{
...
void cps_state::slampic2_sound_w(u16 data)
{
	//logerror("Sound command: %04x\n", data);
}

void cps_state::slampic2_sound2_w(u16 data)
{
...
void cps_state::init_slampic2()
{
	...
	m_bootleg_work_ram = std::make_unique<u16[]>(0x20000);
...
GAME( 1993,  slampic2,   slammast, slampic2,   slampic2, cps_state, init_slampic2,  ROT0,  "bootleg",  "Saturday Night Slam Masters (bootleg with PIC16C57, set 2)",  MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE )  // 930713 ETC


void cps_state::wofpic(machine_config &config)
{
	dinopic(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &cps_state::wofpic_map);
	MCFG_MACHINE_START_OVERRIDE(cps_state, wofpic)
}


void cps_state::wofpic_map(address_map &map) {
	map(0x000000, 0x3fffff).rom();
	map(0x800000, 0x800007).portr("IN1");            /* Player input ports */
	map(0x800006, 0x800007).w(FUNC(cps_state::cps1_soundlatch_w));    /* Sound command */
//	map(0x800008, 0x800009).w(FUNC(cps_state::wofpic_layer2_w));
	map(0x800018, 0x80001f).r(FUNC(cps_state::cps1_dsw_r));            /* System input ports / Dip Switches */
	map(0x800030, 0x800037).w(FUNC(cps_state::cps1_coinctrl_w));
	map(0x800100, 0x80013f).w(FUNC(cps_state::cps1_cps_a_w)).share("cps_a_regs");  /* CPS-A custom */
	map(0x800140, 0x80017f).rw(FUNC(cps_state::cps1_cps_b_r), FUNC(cps_state::cps1_cps_b_w)).share("cps_b_regs");  /* Only writes here at boot */
	map(0x880000, 0x880001).nopw(); // ?
	map(0x900000, 0x92ffff).ram().w(FUNC(cps_state::cps1_gfxram_w)).share("gfxram");
//	map(0x980000, 0x98000d).w(FUNC(cps_state::wofpic_layer_w));
	map(0xf18000, 0xf19fff).nopw(); // few q-sound leftovers
	map(0xf1c000, 0xf1c001).portr("IN2");            /* Player 3 controls (later games) */
	map(0xf1c004, 0xf1c005).w(FUNC(cps_state::cpsq_coinctrl2_w));     /* Coin control2 (later games) */
	map(0xf1c006, 0xf1c007).portr("EEPROMIN").portw("EEPROMOUT");
...
MACHINE_START_MEMBER(cps_state, wofpic)
{
	m_layer_enable_reg = 0x26;
	m_layer_mask_reg[0] = 0x28;
	m_layer_mask_reg[1] = 0x2a;
...
void cps_state::init_wofpic()
{
	m_bootleg_sprite_ram = std::make_unique<u16[]>(0x2000);
	m_maincpu->space(AS_PROGRAM).install_ram(0x990000, 0x993fff, m_bootleg_sprite_ram.get());
//	m_maincpu->space(AS_PROGRAM).install_write_handler(0x990000, 0x990001, write16smo_delegate(*this, FUNC(cps_state::wofpic_spr_base_w)));
	init_cps1();
}
...
GAME( 1992,  wofpic,     wof,       wofpic,    wof,       cps_state,      init_wofpic,    ROT0,  "bootleg",  "Warriors of Fate (bootleg with PIC16C57)",  MACHINE_NOT_WORKING | MACHINE_SUPPORTS_SAVE )  // 921002 ETC
