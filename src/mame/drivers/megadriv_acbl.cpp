...
READ16_MEMBER(md_boot_state::barek2mb_r )
{
...
READ16_MEMBER(md_boot_state::sbubsm_400000_r)
{
	logerror("%s: sbubsm_400000_r\n", machine().describe_context().c_str());
	return 0x5500;
}

READ16_MEMBER(md_boot_state::sbubsm_400002_r)
{
...
void md_boot_state::init_sbubsm()
{
	...
	m_maincpu->space(AS_PROGRAM).install_read_handler(0x400000, 0x400001, read16_delegate(FUNC(md_boot_state::sbubsm_400000_r),this));
	m_maincpu->space(AS_PROGRAM).install_read_handler(0x400002, 0x400003, read16_delegate(FUNC(md_boot_state::sbubsm_400002_r),this));
...
{
	m_maincpu->space(AS_PROGRAM).install_write_handler(0x220000, 0x220001, write16_delegate(FUNC(md_boot_state::aladmdb_w),this));
	m_maincpu->space(AS_PROGRAM).install_read_handler(0x330000, 0x330001, read16_delegate(FUNC(md_boot_state::barek2mb_r),this));
...
GAME( 1994, bk3ssrmb, 0, megadrvb_6b,  bk3ssrmb, md_boot_state, init_bk3ssrmb, ROT0, "bootleg / Sega",   "Bare Knuckle III / Sunset Riders (bootleg of Mega Drive versions)", MACHINE_NOT_WORKING ) // Currently boots as Bare Knuckle III, mechanism to switch game not found yet
...
GAME( 1994, barekch,  0, megadrvb_6b,  barekch,  md_boot_state, init_barekch,  ROT0, "bootleg",          "Bare Knuckle (Chinese bootleg of Mega Drive version)", 0 )
