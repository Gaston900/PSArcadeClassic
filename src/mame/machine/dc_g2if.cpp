...
void dc_g2if_device::dma_execute(u8 channel)
{
	...
	// bulk-copy in dword chunks when aligned: this is the same per-word
	// address_space dispatch bottleneck found in the PVR framebuffer path
	// (see video/powervr2.cpp) — a single voice/narration sample DMA can be
	// tens to hundreds of KB, and was previously one 16-bit read_word +
	// write_word dispatch call pair per 2 bytes.
	while (index < transfer_size && ((src | dst) & 3) == 0 && (transfer_size - index) >= 4)
	{
	...
	for (; dma_result && index < transfer_size; index += 2)
	{
