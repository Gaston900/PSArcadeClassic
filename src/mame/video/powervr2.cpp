...
// direct-pointer helpers: bypass address_space dispatch for VRAM ('frameram')
// access so the render path can run off the main CPU execution thread and
// avoid per-pixel bounds-check/dispatch overhead (same array/XOR convention
// already used by the read side / pvr_drawframebuffer).
static inline uint32_t fb_read32(uint64_t *fb_ram, uint32_t byteoffs)
{
	return *(reinterpret_cast<uint32_t *>(fb_ram) + (WORD2_XOR_LE(byteoffs) >> 2));
}

static inline void fb_write16(uint64_t *fb_ram, uint32_t byteoffs, uint16_t val)
{
	*(reinterpret_cast<uint16_t *>(fb_ram) + (WORD2_XOR_LE(byteoffs) >> 1)) = val;
}

static inline void fb_write24(uint64_t *fb_ram, uint32_t byteoffs, uint32_t val)
{
	*(reinterpret_cast<uint8_t *>(fb_ram) + BYTE8_XOR_LE(byteoffs+0)) = (uint8_t)(val >> 0);
	*(reinterpret_cast<uint8_t *>(fb_ram) + BYTE8_XOR_LE(byteoffs+1)) = (uint8_t)(val >> 8);
	*(reinterpret_cast<uint8_t *>(fb_ram) + BYTE8_XOR_LE(byteoffs+2)) = (uint8_t)(val >> 16);
}

static inline void fb_write32(uint64_t *fb_ram, uint32_t byteoffs, uint32_t val)
{
	*(reinterpret_cast<uint32_t *>(fb_ram) + (WORD2_XOR_LE(byteoffs) >> 2)) = val;
}

// dirty-region y clip for render_band: render_hline checks this instead of
// taking an extra clip parameter through the whole render_tri/render_span
// template chain. (Previously also used for parallel row-band dispatch;
// that was reverted after it caused frame corruption, see render_band.)
static thread_local int t_render_band_y0 = 0;
static thread_local int t_render_band_y1 = 480;
...
// f4 = scaler_ctl
	...
	map(0x0118, 0x011b).rw(FUNC(powervr2_device::y_coeff_r), FUNC(powervr2_device::y_coeff_w));
// 108 = pal_ram_ctrl (mapped above)
// 10c = spg_status (mapped above)
// 110 = fb_burstctrl
// 118 = y_coeff (mapped above)
// 11c = pt_alpha_ref
...
void powervr2_device::startrender_w(address_space &space, uint32_t data)
{
	...
	// a previous render must be fully done before we touch shared scratch
	// state (renderselect/wbuffer/fake_accumulationbuffer_bitmap) again
	wait_render_done();
	...
	for (int a=0;a < NUM_BUFFERS;a++) {
		if ((grab[a].ispbase == param_base) && (grab[a].valid == 1) && (grab[a].busy == 0)) {
			...
			// FB_W_SOF1 bit 24 signals render-to-texture (see render_target_ram);
			// strip it before using fbwsof1 as a byte offset or it reads as a
			// bogus 16MB+ address into dc_framebuffer_ram/dc_texture_ram.
			grab[a].is_rtt_snap = (fb_w_sof1 & 0x1000000) != 0;
			grab[a].fbwsof1 = fb_w_sof1 & ~0x1000000;
			grab[a].fbwsof2 = fb_w_sof2 & ~0x1000000;

			// snapshot everything the worker thread will read, so a
			// subsequent register write (already possible once we return)
			// can't corrupt an in-flight render (see receiveddata comment)
			grab[a].region_base_snap = region_base;
			grab[a].param_base_snap = param_base;
			grab[a].fpu_param_cfg_snap = fpu_param_cfg;
			grab[a].isp_backgnd_t_snap = isp_backgnd_t;
			grab[a].fb_w_ctrl_snap = fb_w_ctrl;
			grab[a].fb_r_ctrl_snap = fb_r_ctrl;
			grab[a].fb_w_linestride_snap = fb_w_linestride;

			// kick the actual rasterization off to a worker thread so the SH4
			// doesn't stall mid-instruction (see render_run/render_worker_callback);
			// the EOR IRQ timer below waits on this job before firing.
			m_render_item = osd_work_item_queue(m_render_queue, render_worker_callback, this, 0);

			/* Fire ISP irq after a set amount of time */
			// data-proportional delay (flycast scheduleRenderDone model): scales with
			// actual TA payload bytes instead of a fixed/tile-count-only hack, so
			// EOR IRQ latency tracks real scene complexity like real hardware/flycast.
			const u64 isp_completion = std::min<u64>(450000 + (u64)grab[renderselect].data_size * 100, 1500000);
...
uint32_t powervr2_device::y_coeff_r()
{
	return y_coeff;
}

void powervr2_device::y_coeff_w(offs_t offset, uint32_t data, uint32_t mem_mask)
{
	...
	{ // global parameter or vertex parameter
		...
		{ // vertex
				...
				(tafifo_listtype == DISPLAY_LIST_TRANS_MOD))
			{
				...
				// a modifier volume vertex parameter carries a whole triangle
				// (A,B,C) at once instead of one strip vertex at a time; store
				// it as its own 3-vertex mini-strip so the existing strip/tri
				// walk in render_group_to_accumulation_buffer renders it as a
				// single triangle (see render_hline's *_MOD branch).
				if (rd->verts_size <= (MAX_VERTS - 3) && grp->strips_size < MAX_STRIPS)
				{
					vert *tv = &rd->verts[rd->verts_size];
					tv[0].x = u2f(tafifo_buff[1]); tv[0].y = u2f(tafifo_buff[2]); tv[0].w = u2f(tafifo_buff[3]);
					tv[1].x = u2f(tafifo_buff[4]); tv[1].y = u2f(tafifo_buff[5]); tv[1].w = u2f(tafifo_buff[6]);
					tv[2].x = u2f(tafifo_buff[7]); tv[2].y = u2f(tafifo_buff[8]); tv[2].w = u2f(tafifo_buff[9]);
					for (int idx = 0; idx < 3; idx++) {
						tv[idx].u = 0.0f;
						tv[idx].v = 0.0f;
						memset(tv[idx].b, 0, sizeof(tv[idx].b));
						memset(tv[idx].o, 0, sizeof(tv[idx].o));
					}

					strip *ts = &grp->strips[grp->strips_size++];
					tex_get_info(&ts->ti);
					ts->svert = rd->verts_size;
					ts->evert = rd->verts_size + 2;
...
inline void powervr2_device::render_hline(bitmap_rgb32 &bitmap, texinfo *ti, int y, float xl, float xr, float ul, float ur, float vl, float vr, float wl, float wr, float const bl_in[4], float const br_in[4], float const offl_in[4], float const offr_in[4])
{
	...
	if (y < t_render_band_y0 || y >= t_render_band_y1)
		return;
	...
	// modifier volume groups don't draw color: they only toggle an even-odd
	// crossing count wherever the volume surface is occluded by the already
	// rendered opaque/translucent depth (see apply_modifier_volume for how
	// this gets folded into an actual darkening effect).
	if (group_no == DISPLAY_LIST_OPAQUE_MOD || group_no == DISPLAY_LIST_TRANS_MOD)
	{
		int16_t *cline = &modvol_count[y][xxl];
		while (xxl < xxr) {
			if (wl < *wbufline)
				(*cline) ^= 1;
			cline++;
...
void powervr2_device::render_band(bitmap_rgb32 &bitmap, const rectangle &cliprect, int y0, int y1)
{
	t_render_band_y0 = y0;
	t_render_band_y1 = y1;

	// only clear the w-buffer region that's actually going to be rendered
	// into this band, instead of the whole 640x480 every render.
	int clear_y0 = std::max(y0, 0);
	int clear_y1 = std::min(y1, 480) - 1;
	int clear_x0 = std::max(cliprect.left(), 0);
	int clear_x1 = std::min(cliprect.right(), 639);
	if (clear_x1 >= clear_x0)
		for (int y = clear_y0; y <= clear_y1; y++)
		{
			memset(&wbuffer[y][clear_x0], 0, (size_t)(clear_x1 - clear_x0 + 1) * sizeof(wbuffer[0][0]));
			memset(&modvol_area[y][clear_x0], 0, (size_t)(clear_x1 - clear_x0 + 1) * sizeof(modvol_area[0][0]));
		}

	// TODO: read ISP/TSP command from isp_background_t instead of assuming Gourad-shaded
	// full-screen polygon.
	uint32_t c = fb_read32(dc_framebuffer_ram, (grab[renderselect].param_base_snap&0xf00000)+((grab[renderselect].isp_backgnd_t_snap&0xfffff8)>>1)+(3+3)*4);
	rectangle band_clip(cliprect.left(), cliprect.right(), y0, y1-1);
	bitmap.fill(c, band_clip);

	// draw order matches real hardware/flycast (opaque, then punch-through,
	// then translucent last): punch-through was previously drawn after
	// translucent, letting alpha-blended layers (smoke/explosions) wrongly
	// cover punch-through content (e.g. UI/ranking screens) instead of
	// vice versa.
	render_group_to_accumulation_buffer<DISPLAY_LIST_OPAQUE>(bitmap, band_clip);
	render_group_to_accumulation_buffer<DISPLAY_LIST_PUNCH_THROUGH>(bitmap, band_clip);

	// modifier volumes: opaque/punch-through's affects that pair, translucent's
	// affects only the translucent list (see apply_modifier_volume).
	render_group_to_accumulation_buffer<DISPLAY_LIST_OPAQUE_MOD>(bitmap, band_clip);
	apply_modifier_volume(bitmap, band_clip);

	render_group_to_accumulation_buffer<DISPLAY_LIST_TRANS>(bitmap, band_clip);

	render_group_to_accumulation_buffer<DISPLAY_LIST_TRANS_MOD>(bitmap, band_clip);
	apply_modifier_volume(bitmap, band_clip);
}

void powervr2_device::apply_modifier_volume(bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	int x0 = std::max(cliprect.left(), 0);
	int x1 = std::min(cliprect.right(), 639);
	int y0 = std::max(cliprect.top(), 0);
	int y1 = std::min(cliprect.bottom(), 479);
	if (x1 < x0 || y1 < y0)
		return;

	for (int y = y0; y <= y1; y++)
	{
		int16_t *cline = &modvol_count[y][x0];
		uint8_t *aline = &modvol_area[y][x0];
		uint32_t *pix = &bitmap.pix(y, x0);
		for (int x = x0; x <= x1; x++, cline++, aline++, pix++)
		{
			// even-odd rule: an odd crossing count means this pixel is
			// inside the (simplified, single) modifier volume
			if (*cline & 1)
				*aline = 1;
			*cline = 0;
			if (*aline)
...
void powervr2_device::render_to_accumulation_buffer(bitmap_rgb32 &bitmap, const rectangle &cliprect) {
	...
	// dirty-region clear/fill confirmed NOT the cause of the ranking-screen
	// stale-content bug (tested with a forced full-screen clip, no change) ??
	// restored the tile-bbox optimization.
	int y0 = std::max(cliprect.top(), 0);
	int y1 = std::min(cliprect.bottom() + 1, 480);
	if (y1 <= y0)
	{
	...
	render_band(bitmap, cliprect, y0, y1);
...
void powervr2_device::fb_convert_0555krgb_to_555rgb(int x,int y)
{
	...
	{
		uint32_t fboffs = grab[renderselect].fbwsof1 + (y+ycnt) * (grab[renderselect].fb_w_linestride_snap<<3) + (x*2);
		...
		for (int xcnt=0;xcnt<32;xcnt++)
		{
			...
			fb_write16(render_target_ram(), fboffs+xcnt*2, newdat);
		}
	}
}
...
void powervr2_device::fb_convert_0555krgb_to_888rgb24(int x,int y)
{
	for (int ycnt=0;ycnt<32;ycnt++)
	{
		uint32_t fboffs = grab[renderselect].fbwsof1 + (y+ycnt) * (grab[renderselect].fb_w_linestride_snap<<3) + (x*3);
		uint32_t const *const src = &fake_accumulationbuffer_bitmap->pix(y+ycnt, x);
		...
		for (int xcnt=0;xcnt<32;xcnt++)
		{
			...
			fb_write24(render_target_ram(), fboffs+xcnt*3, newdat);
		}
	}
}
...
void powervr2_device::fb_convert_0555krgb_to_888rgb32(int x,int y)
{
	for (int ycnt=0;ycnt<32;ycnt++)
	{
		uint32_t fboffs = grab[renderselect].fbwsof1 + (y+ycnt) * (grab[renderselect].fb_w_linestride_snap<<3) + (x*4);
		uint32_t const *const src = &fake_accumulationbuffer_bitmap->pix(y+ycnt, x);
		...
		for (int xcnt=0;xcnt<32;xcnt++)
		{
			...
			fb_write32(render_target_ram(), fboffs+xcnt*4, newdat);
		}
	}
}
...
void powervr2_device::pvr_accumulationbuffer_to_framebuffer(int x,int y)
{
	...
	uint8_t packmode = grab[renderselect].fb_w_ctrl_snap & 0x7;
	uint8_t unpackmode = (grab[renderselect].fb_r_ctrl_snap & 0x0000000c) >>2;  // aka fb_depth
...
{
	// the scheduled delay is normally plenty of time for the worker to have
	// finished; this just guards against the rare case it hasn't.
	wait_render_done();
...
uint32_t powervr2_device::screen_update(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	******************/
	...
	// don't reap the async render worker with a blocking wait here (see
	// reap_render_if_done): only pick it up if it's already done, otherwise
	// just display whatever is currently in the framebuffer this frame.
	reap_render_if_done();
...
// blocks until any in-flight async render job has finished; must be called
// before touching shared render scratch state (renderselect/wbuffer/etc.)
// or before a save/rewind snapshot.
void powervr2_device::wait_render_done()
{
	if (m_render_item)
	{
		osd_work_item_wait(m_render_item, osd_ticks_per_second() * 10);
		osd_work_item_release(m_render_item);
		m_render_item = nullptr;
	}
}

// non-blocking: reaps the render item only if it has already finished.
// screen_update() must NEVER stall on the render worker: MAME's cooperative
// device_scheduler runs sound_manager::update()'s periodic stream timer on
// the exact same thread, so any blocking wait here delays audio generation
// too and is heard as stutter (and, transitively, throttles frame pacing).
// A stale/partially-updated framebuffer for one displayed frame is a much
// smaller artifact than that.
void powervr2_device::reap_render_if_done()
{
	if (m_render_item && osd_work_item_wait(m_render_item, 0))
	{
		osd_work_item_release(m_render_item);
		m_render_item = nullptr;
	}
...
// runs on the render worker thread: rasterize into the accumulation buffer,
// then blit the tiles into the framebuffer, exactly like the old synchronous
// code path did, just off the SH4 execution thread.
void powervr2_device::render_run()
{
	int sizera = grab[renderselect].fpu_param_cfg_snap & 0x200000 ? 6 : 5;
	int offsetra = grab[renderselect].region_base_snap;

	// dirty-region pass: walk the region list once to collect the tiles
	// actually submitted this frame and their bounding box, so the
	// accumulation-buffer clear/fill below only touches what's really used
	// instead of redoing the whole 1024x1024 buffer on every render (list
	// pointer words st[1..5] are never used beyond logging, so skip reading them).
	std::vector<std::pair<int16_t, int16_t>> tiles;
	int minx = INT_MAX, miny = INT_MAX, maxx = 0, maxy = 0;

	// sanity: prevent infinite loop if asked to process invalid data
	for (int sanitycount = 0; sanitycount < 100000; sanitycount++)
	{
		uint32_t st0 = fb_read32(dc_framebuffer_ram, offsetra);
		offsetra += (sizera == 6) ? 0x18 : 0x14;

		int x = ((st0&0x000000fc)>>2)*32;
		int y = ((st0&0x00003f00)>>8)*32;

		tiles.emplace_back((int16_t)x, (int16_t)y);
		minx = std::min(minx, x);
		miny = std::min(miny, y);
		maxx = std::max(maxx, x+32);
		maxy = std::max(maxy, y+32);
	...
	if (!tiles.empty())
		clip = rectangle(minx, maxx-1, miny, maxy-1);
	...
	for (const auto &t : tiles)
		pvr_accumulationbuffer_to_framebuffer(t.first, t.second);
}

void *powervr2_device::render_worker_callback(void *param, int threadid)
{
	static_cast<powervr2_device *>(param)->render_run();
	return nullptr;
}
