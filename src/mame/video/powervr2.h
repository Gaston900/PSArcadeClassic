...
public:
	...
	// modifier volume support (see render_hline/apply_modifier_volume):
	// modvol_count is an even-odd crossing counter rebuilt per volume group,
	// folded into modvol_area (1 = pixel darkened by a shadow/mod volume).
	// Simplified vs. real hardware/flycast: single even-odd parity rule (no
	// separate inclusion/exclusion CVS combination, no per-polygon Shadow-bit
	// gating, no Naomi2 matrices) but captures the common single-volume case.
	int16_t modvol_count[480][640];
	uint8_t modvol_area[480][640];
	...
	struct receiveddata {
		...
		int valid = 0;
		int verts_size = 0;
		uint32_t data_size = 0; // TA payload bytes received for this context, for EOR IRQ timing

		// registers snapshotted at STARTRENDER time: the async render worker
		// (render_run) must not read the live device members directly, since
		// the SH4 can rewrite them (e.g. for the next TA context) before the
		// worker thread gets to consume them.
		uint32_t region_base_snap = 0;
		uint32_t param_base_snap = 0;
		uint32_t fpu_param_cfg_snap = 0;
		uint32_t isp_backgnd_t_snap = 0;
		uint32_t fb_w_ctrl_snap = 0;
		uint32_t fb_r_ctrl_snap = 0;
		uint32_t fb_w_linestride_snap = 0;
		bool is_rtt_snap = false; // render-to-texture: FB_W_SOF1 bit 24 (see render_target_ram)
	};
	...
	uint32_t y_coeff_r();
	void y_coeff_w(offs_t offset, uint32_t data, uint32_t mem_mask = ~0);
...
private:
	...
	// async render worker (see render_run/render_worker_callback): the heavy
	// per-tile rasterization kicked off by STARTRENDER runs on this queue so
	// the SH4 execution thread isn't blocked and can keep feeding the AICA/ARM7
	// its scheduler quantum. Only one job may be in flight at a time; a new
	// STARTRENDER waits for the previous job before touching shared scratch
	// state (renderselect/wbuffer/fake_accumulationbuffer_bitmap).
	osd_work_queue *m_render_queue = nullptr;
	osd_work_item *m_render_item = nullptr;
	void wait_render_done();
	void reap_render_if_done();

	// per-render dirty-region clear/fill helper (see render_to_accumulation_buffer);
	// y0/y1 bound how much of wbuffer/the accumulation bitmap actually needs
	// clearing this pass instead of always doing the full 640x480/1024x1024.
	void render_band(bitmap_rgb32 &bitmap, const rectangle &cliprect, int y0, int y1);

	// darkens bitmap pixels within cliprect where modvol_area is set, then
	// clears modvol_area/modvol_count for the next group (see render_band).
	void apply_modifier_volume(bitmap_rgb32 &bitmap, const rectangle &cliprect);

	// render-to-texture support: real hardware/flycast signal RTT via bit 24
	// of FB_W_SOF1 (fb_w_sof1), in which case rendered pixels must land in
	// texture memory (sampled later by a textured poly) instead of the
	// display framebuffer. Without this, a set bit 24 would also make every
	// fb_w_sof1-based address 16MB+ out of range for dc_framebuffer_ram.
	uint64_t *render_target_ram() const { return grab[renderselect].is_rtt_snap ? dc_texture_ram : dc_framebuffer_ram; }
	...
	uint32_t y_coeff; // Y scaling coefficient (0x118), real HW is RW but otherwise unused by our renderer
...
//  uint32_t tex_r_yuv_vq(texinfo *t, float x, float y);
	...
	void render_run(); // heavy per-render work, executed off the main thread
	static void *render_worker_callback(void *param, int threadid);
