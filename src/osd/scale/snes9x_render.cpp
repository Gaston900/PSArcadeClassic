...
static uint16_t RGBtoBright[1<<NUMBITS];
...
#define InitLine(topValid, botValid, scale, diags)                \
	{                                                             \
		if(topValid) uP  = (uint16_t *) (srcPtr - srcPitch);        \
		if(botValid) lP  = (uint16_t *) (srcPtr + srcPitch);        \
					 sP  = (uint16_t *) srcPtr;                     \
					 dP1 = (destType *) dstPtr;                   \
...
#define DrawInit(scale,uintDest)                                                            \
	...
	const uint32_t srcPitch = srcpitch, dstPitch = dstpitch;                                \
	const uint32_t srcHeight = nHeight;                              \
	const uint32_t srcWidth = nWidth;                               \
	uint16_t	colorX, colorA, colorB, colorC, colorD, colorE=0, colorF=0, colorG=0, colorH=0; \
	uint16_t	*sP, *uP, *lP;                                                                  \
...
void RenderEPXC(unsigned char *src, unsigned int srcpitch, unsigned char *dst, unsigned int dstpitch, int nWidth, int nHeight)
{
	...
	{                                    /* on10 on11 */ \
		if ((((on00||on10)?colorA:colorX) != ((on01||on11)?colorC:colorX)) \
		 && (((on10||on11)?colorB:colorX) != ((on00||on01)?colorD:colorX))) \
		{ \
			uint8_t XnE = colorX != colorE; \
			uint8_t XnF = colorX != colorF; \
			uint8_t XnG = colorX != colorG; \
			uint8_t XnH = colorX != colorH; \
			uint8_t DA = on00 && colorD == colorA && (XnE || XnG || colorD != colorH || colorA != colorF); \
			uint8_t AB = on10 && colorA == colorB && (XnF || XnH || colorA != colorE || colorB != colorG); \
			uint8_t BC = on11 && colorB == colorC && (XnG || XnE || colorB != colorF || colorC != colorH); \
			uint8_t CD = on01 && colorC == colorD && (XnH || XnF || colorC != colorG || colorD != colorE); \
			...
			{ \
				const uint16_t colorAA = on00&&on10 && ((DA && XnF) || (AB && XnE)) ? colorA : colorX; \
				const uint16_t colorBB = on10&&on11 && ((AB && XnG) || (BC && XnF)) ? colorB : colorX; \
				const uint16_t colorCC = on01&&on11 && ((BC && XnH) || (CD && XnG)) ? colorC : colorX; \
				const uint16_t colorDD = on00&&on01 && ((CD && XnE) || (DA && XnH)) ? colorD : colorX; \
...
inline uint8_t Diff(int c1, int c2)
{
...
void RenderHQ2X(unsigned char *src, unsigned int srcpitch, unsigned char *dst, unsigned int dstpitch, int nWidth, int nHeight, int GuiScale)
{
	uint8_t *srcPtr = src;
	uint8_t *dstPtr = dst;
	uint32_t srcPitch = srcpitch;
	uint32_t dstPitch = dstpitch;
	int width = nWidth;
	int height = nHeight;

	int	w1, w2, w3, w4, w5, w6, w7, w8, w9;

	// Corregido: MAME 0.289 entrega los pitches limpios en pixeles. Eliminar el desplazamiento '>> 1'
	// Corrección definitiva para ajustar el aspecto a pantalla completa en 0.289
	uint32_t	src1line = srcPitch >> 1; // Convierte bytes crudos a shorts (16 bits)
	uint32_t	dst1line = dstPitch >> 1; // Convierte bytes crudos a shorts (16 bits)
	...
	{
    	uint16_t * sp0 = sp - src1line;
	    if (h == 0 && sp0 < (uint16_t *) srcPtr)
	          sp0 = sp;
		...
		for (l = width; l; l--)
		{
			...
			{
			case 0: {
				const uint16_t avg = (RGBtoBright[w1] + RGBtoBright[w2] + RGBtoBright[w3] + RGBtoBright[w4] + RGBtoBright[w5] + RGBtoBright[w6] + RGBtoBright[w7] + RGBtoBright[w8] + RGBtoBright[w9]) / 9;
				uint8_t diff5 = RGBtoBright[w5] > avg;
