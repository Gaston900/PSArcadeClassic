...
#include <math.h>
#include <assert.h>
//#include <algorithm> // Cabecera estándar para std::min y std::max

// Eliminamos las macros manuales que chocan con el núcleo de MAME
#if defined(MAX)
#undef MAX
#endif
#if defined(MIN)
...
namespace
{
template <xbrz_uint32 N> inline
unsigned char getByte(xbrz_uint32 val) { return static_cast<unsigned char>((val >> (8 * N)) & 0xff); }

inline unsigned char getRed  (xbrz_uint32 val) { return getByte<2>(val); }
inline unsigned char getGreen(xbrz_uint32 val) { return getByte<1>(val); }
inline unsigned char getBlue (xbrz_uint32 val) { return getByte<0>(val); }
...
const xbrz_uint32 redMask   = 0xff0000;
const xbrz_uint32 greenMask = 0x00ff00;
const xbrz_uint32 blueMask  = 0x0000ff;

template <unsigned int N, unsigned int M> inline
void alphaBlend(xbrz_uint32& dst, xbrz_uint32 col) //blend color over destination with opacity N / M
{
...
inline xbrz_uint32 alphaBlend2(xbrz_uint32 pix1, xbrz_uint32 pix2, double alpha)
{
    return (redMask   & static_cast<xbrz_uint32>((pix1 & redMask  ) * alpha + (pix2 & redMask  ) * (1 - alpha))) |
           (greenMask & static_cast<xbrz_uint32>((pix1 & greenMask) * alpha + (pix2 & greenMask) * (1 - alpha))) |
           (blueMask  & static_cast<xbrz_uint32>((pix1 & blueMask ) * alpha + (pix2 & blueMask ) * (1 - alpha)));
}


xbrz_uint32*       byteAdvance(      xbrz_uint32* ptr, int bytes) { return reinterpret_cast<      xbrz_uint32*>(reinterpret_cast<      char*>(ptr) + bytes); }
const xbrz_uint32* byteAdvance(const xbrz_uint32* ptr, int bytes) { return reinterpret_cast<const xbrz_uint32*>(reinterpret_cast<const char*>(ptr) + bytes); }


//fill block  with the given color
inline void fillBlock(xbrz_uint32* trg, int pitch, xbrz_uint32 col, int blockWidth, int blockHeight)
{
...
inline void fillBlock(xbrz_uint32* trg, int pitch, xbrz_uint32 col, int n) { fillBlock(trg, pitch, col, n, n); }
...
{
public:
    OutputMatrix(xbrz_uint32* out, int outWidth) : //access matrix area, top-left at position "out" for image with given width
        out_(out),
        outWidth_(outWidth) {}

    template <size_t I, size_t J>
    xbrz_uint32& ref() const
    {
...
/*
inline
void rgbtoLuv(xbrz_uint32 c, double& L, double& u, double& v)
{
...
inline void rgbtoLab(xbrz_uint32 c, unsigned char& L, signed char& A, signed char& B)
{
...
inline double distLAB(xbrz_uint32 pix1, xbrz_uint32 pix2)
{
...
/*
inline
void rgbtoHsl(xbrz_uint32 c, double& h, double& s, double& l)
{
...
inline
double distHSL(xbrz_uint32 pix1, xbrz_uint32 pix2, double lightningWeight)
{
    double h1 = 0;
    double s1 = 0;
    double l1 = 0;
...
inline double distRGB(xbrz_uint32 pix1, xbrz_uint32 pix2)
{
...
inline double distNonLinearRGB(xbrz_uint32 pix1, xbrz_uint32 pix2)
{
...
inline double distYCbCr(xbrz_uint32 pix1, xbrz_uint32 pix2, double lumaWeight)
{
...
inline double distYUV(xbrz_uint32 pix1, xbrz_uint32 pix2, double luminanceWeight)
{
...
inline double colorDist(xbrz_uint32 pix1, xbrz_uint32 pix2, double luminanceWeight)
{
...
inline bool equalColor(xbrz_uint32 col1, xbrz_uint32 col2, double luminanceWeight, double equalColorTolerance)
{
...
BlendResult preProcessCorners(const Kernel_4x4& ker, const XbrzScalerCfg& cfg) //result: F, G, J, K corners of "GradientType"
{
    BlendResult result = {};
...
#define DEF_GETTER(x) template <RotationDegree rotDeg> xbrz_uint32 inline get_##x(const Kernel_3x3& ker) { return ker.x; }
...
#define DEF_GETTER(x, y) template <> inline xbrz_uint32 get_##x<ROT_90>(const Kernel_3x3& ker) { return ker.y; }
...
#define DEF_GETTER(x, y) template <> inline xbrz_uint32 get_##x<ROT_180>(const Kernel_3x3& ker) { return ker.y; }
...
#define DEF_GETTER(x, y) template <> inline xbrz_uint32 get_##x<ROT_270>(const Kernel_3x3& ker) { return ker.y; }
...
-------------
*/
#define eq(col1, col2) equalColor(col1, col2, cfg.luminanceWeight_, cfg.equalColorTolerance_)
#define dist(col1, col2) colorDist(col1, col2, cfg.luminanceWeight_)

template <class Scaler, RotationDegree rotDeg>
FORCE_INLINE //perf: quite worth it!
void scalePixel(const Kernel_3x3& ker,
                xbrz_uint32* target, int trgWidth,
                unsigned char blendInfo, //result of preprocessing all four corners of pixel "e"
                const XbrzScalerCfg& cfg)
{
...
#define i get_i<rotDeg>(ker)
    ...
    if (getBottomR(blend) >= BLEND_NORMAL)
    {
        ...
        const xbrz_uint32 px = dist(e, f) <= dist(e, h) ? f : h; //choose most similar color
...
void scaleImage(const xbrz_uint32* src, xbrz_uint32* trg, int srcWidth, int srcHeight, const XbrzScalerCfg& cfg, int yFirst, int yLast, int srcPitch, int trgPitch)
{
    yFirst = MAX(yFirst, 0);
...
//    const int trgWidth = srcWidth * Scaler::scale;
    ...
    //"sizeof(xbrz_uint32) * srcWidth * (yLast - yFirst)" bytes without risk of accidental overwriting before accessing
    const int bufferSize = srcWidth;
    unsigned char* preProcBuffer = reinterpret_cast<unsigned char*>(trg + yLast * trgPitch / 4) - bufferSize;
    ...
    if (yFirst > 0)
    {
        ...
        const xbrz_uint32* s_m1 = src +  srcPitch / 4 * MAX(y - 1, 0);
        const xbrz_uint32* s_0  = src +  srcPitch / 4 * y; //center line
        const xbrz_uint32* s_p1 = src +  srcPitch / 4 * MIN(y + 1, srcHeight - 1);
        const xbrz_uint32* s_p2 = src +  srcPitch / 4 * MIN(y + 2, srcHeight - 1);
    ...
    {
        //xbrz_uint32* out = trg + Scaler::scale * y * trgWidth; //consider MT "striped" access
        xbrz_uint32* out = trg + Scaler::scale * y * trgPitch/4; //consider MT "striped" access
        ...
        for (int x = 0; x < srcWidth; ++x, out += Scaler::scale)
        {
            ...
            fillBlock(out, trgPitch, s_0[x], Scaler::scale); //place *after* preprocessing step, to not overwrite the results while processing the the last pixel!
            ...
            if (blendingNeeded(blend_xy)) //good 20% perf-improvement
            {
                ...
                scalePixel<Scaler, ROT_0  >(ker, out, trgPitch / 4, blend_xy, cfg);
                scalePixel<Scaler, ROT_90 >(ker, out, trgPitch / 4, blend_xy, cfg);
                scalePixel<Scaler, ROT_180>(ker, out, trgPitch / 4, blend_xy, cfg);
                scalePixel<Scaler, ROT_270>(ker, out, trgPitch / 4, blend_xy, cfg);
            }
        }
    }
}
...
struct Scaler2x
{
    ...
    static void blendLineShallow(xbrz_uint32 col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineSteep(xbrz_uint32 col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineSteepAndShallow(xbrz_uint32 col, OutputMatrix& out)
    {
    ...
    template <class OutputMatrix>
    static void blendLineDiagonal(xbrz_uint32 col, OutputMatrix& out)
    {
        alphaBlend<1, 2>(out.template ref<1, 1>(), col);
    }

    template <class OutputMatrix>
    static void blendCorner(xbrz_uint32 col, OutputMatrix& out)
    {
...
void xbrz_scale(size_t factor, const xbrz_uint32* src, xbrz_uint32* trg, int srcWidth, int srcHeight, const XbrzScalerCfg& cfg, int yFirst, int yLast, int srcPitch, int trgPitch)
{
    switch (factor)
    {
        case 2:
            return scaleImage<Scaler2x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast, srcPitch, trgPitch);
        case 3:
            return scaleImage<Scaler3x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast, srcPitch, trgPitch);
        case 4:
            return scaleImage<Scaler4x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast, srcPitch, trgPitch);
        case 5:
            return scaleImage<Scaler5x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast, srcPitch, trgPitch);
    }
//    assert(false);
}
...
{
	return equalColor(col1, col2, luminanceWeight, equalColorTolerance);
}


void nearestNeighborScale(const xbrz_uint32* src, int srcWidth, int srcHeight, int srcPitch,
                                xbrz_uint32* trg, int trgWidth, int trgHeight, int trgPitch,
                                SliceType st, int yFirst, int yLast)
{
    if (srcPitch < srcWidth * static_cast<int>(sizeof(xbrz_uint32))  ||
        trgPitch < trgWidth * static_cast<int>(sizeof(xbrz_uint32)))
    {
//        assert(false);
        return;
    }
    ...
    switch (st)
    {
        case NN_SCALE_SLICE_SOURCE:
            ...
            for (int y = yFirst; y < yLast; ++y)
            {
                ...
                if (blockHeight > 0)
                {
                    const xbrz_uint32* srcLine = byteAdvance(src, y * srcPitch);
                    xbrz_uint32* trgLine  = byteAdvance(trg, yTrg_first * trgPitch);
                    int xTrg_first = 0;
        ...
        case NN_SCALE_SLICE_TARGET:
            ...
            for (int y = yFirst; y < yLast; ++y)
            {
                xbrz_uint32* trgLine = byteAdvance(trg, y * trgPitch);
                const int ySrc = srcHeight * y / trgHeight;
                const xbrz_uint32* srcLine = byteAdvance(src, ySrc * srcPitch);
...
{
	xbrz_scale(2, (xbrz_uint32*)src, (xbrz_uint32*)dst, nWidth, nHeight, cfg, 0, nHeight, srcPitch, trgPitch);
}

void Render3xXbrz(unsigned char *src, int srcPitch, unsigned char *dst, int trgPitch, int nWidth, int nHeight)
{
	xbrz_scale(3, (xbrz_uint32*)src, (xbrz_uint32*)dst, nWidth, nHeight, cfg, 0, nHeight, srcPitch, trgPitch);
}
